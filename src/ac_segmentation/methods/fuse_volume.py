                    ...
                    if len(write_objects) > 0:
                        ...
                        for attempt in range(max_retries):
                            try:
                                ...
                                # --- crop ---
                                tempx, tempy, tempz = temp_arr.shape[-3:]
                                tx1, ty1, tz1, tx2, ty2, tz2 = 0, 0, 0, tempx, tempy, tempz
                                if crop:
                                    cx, cy, cz = crop
                                    sx, sy, sz = start[i][-3:]
                                    ex, ey, ez = end[i][-3:]
                                ...
                                # --- blend + write ---
                                if is_5d:
                                    temp_arr = temp_arr[:, :, tx1:tx2, ty1:ty2, tz1:tz2]
                                    if blend:
...
class FusionZarrParameters(argschema.ArgSchema):
    ...
    blend = argschema.fields.Bool(required=False, default=False)
    crop = argschema.fields.List(argschema.fields.Int(), required=False, default=None, allow_none=True)
...
class FusionZarrModule(argschema.ArgSchemaParser):
    ...
    def run(self):
        ...
        crop = tuple(self.args['crop']) if self.args['crop'] else None

        fuse_gunpowder(
            arrs=arrays,
            translations=translations,
            output_path=os.path.join(self.args['output_path'], str(self.args['mip'])),
            batch_size=5,
            iter_size=(64, 64, 64),
            run_exclusive=self.args['run_exclusive'],
